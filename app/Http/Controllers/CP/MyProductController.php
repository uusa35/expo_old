...
class MyProductController extends Controller
{
    ...
    {
        $userInfo = Expo::query()->where('user_id',Auth::id())->first();
        $items = Product::query()->where('expo_id',$userInfo->id);
        ...
        return view('cp.myproducts.home', [
            'items' => $items,
        ]);
    ...
    public function create()
    {
      ...
      //  $publishes = Publish::all();
        return view('cp.myproducts.create', [
...
//        return $request->all();
      ...
      /* if ($request->has('is_sale')) {
        ...
        $item->save();
         ...
         // return "dsadsadas".count($request->file('product_images'));
          if(!empty($request->file('product_images')) && count($request->file('product_images')) > 0){
    ...
    public function edit($id)
    {
        ...
        return view('cp.myproducts.edit', [
            'categories' => $categories,
            'item' => $item,
            'sizes' => $sizes,
            'clothing_types' => $clothing_types,
    ...
    public function update(Request $request, $id)
    {
        ...
        //$item->old_price = $request->get('old_price');
        $item->category_id = 1;
